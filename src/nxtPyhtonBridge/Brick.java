...
public class Brick {
	...
	// Basis Funktionen //
	public Brick(String name, boolean fake) throws Exception {
		this.name = name;
		this.fake = fake;
		if(fake){return;}
		...
		if (debug) {
			System.out.println(name + "(d): start with " + pathToExe);
		}
		...
		if (!readStream().equals("ready")) {
			throw new Exception();
		}
		;
		writeStream(name);
		if (!readStream().equals("connecting")) {
			throw new Exception();
		}
		;
		...
		;
		if (debug) {
			System.out.println(name + "(d): ready to control :)");
		}
	}
	...
	protected void start() throws Exception {
		...
		if (debug) {
			System.out.println(name + "(d): start_nxc");
		}
		if (!readStream().equals("success")) {
			throw new Exception();
		}
	...
	protected void stop() throws Exception {
		...
		if (debug) {
			System.out.println(name + "(d): stop_nxc");
		}
		if (!readStream().equals("success")) {
			throw new Exception();
		}
	...
	protected void writeMailbox(int which, int what, int answer)
			throws Exception {
		if (which < 0 || which > 9) {
			return;
		}
		...
		if (debug) {
			System.out.println(name + "(d): send " + what + " on " + which);
		}
		if (!readStream().equals("success")) {
			throw new Exception();
		}
		if (answer >= 0 && answer <= 9) {
			readMailbox(answer, true, -1);
		}
	}

	// Lese etwas aus der Mailbox und warte auf eine Antwort eventuell
	protected int readMailbox(int which, boolean wait, int answer)
			throws Exception {
		if (which < 0 || which > 9) {
			return -1;
		}
		...
		if (wait) {
			if (debug) {
				System.out.println(name + "(d): wait for message on " + which);
			}
		}
		do {
		...
		if (out.equals("nomessage")) {
			out = "0";
		}
		int output = Integer.parseInt(out.replaceAll("\\D+", "")) - 1;
		if (debug) {
			System.out.println(name + "(d): read " + output + " on " + which);
		}
		if (answer >= 0 && answer <= 9 && output != -1) {
			writeMailbox(answer, 0, -1);
		}
		return output;
	}
